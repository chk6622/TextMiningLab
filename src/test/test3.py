...
def get_mean_vector(sents):
    '''
    get the mean vector of sentences
    @param sents:a list of sentences
    @return: sentences vector's mean value 
    '''
    a=np.zeros(300)
    for sent in sents:
        a=a+sent.vector
...
def get_central_vector(sents):
    '''
    get the sentence which is the closest to the central
    @param sents:a list of sentences
    @return: the sentence which is the closest to the central
    '''
    vecs=[]
    for sent in sents:
...
def extract_core_sents(sents):
    ...
    @param sents:a list of sentences
    @return: a sentence list, every sentence is the core sentence which is in every topic cluster
    '''
    s_return=[]
    vectors=[]
    sentences=[]
    ...
    for ind,count in count_result:
        ...
        s_return.append(get_central_vector(sent_cluster))
    return s_return
